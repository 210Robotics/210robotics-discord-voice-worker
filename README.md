...
This always-on worker powers the `/record` command, website-backed message logging, and the verified check-mark reaction for the 210 Robotics Discord server. It runs separately because Discord voice requires a persistent Gateway WebSocket and UDP media connection.
...
5. Copy the resulting Render URL and the same secret into the website's Vercel production environment:
